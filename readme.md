...
To connect to Slack, the app uses a Legacy Slack API token. This can be found at https://api.slack.com/custom-integrations/legacy-tokens and must be added to your environment variables, after generating it for your target team. To do this, copy it and run the command `$ export SLACK_TOKEN='[your token here]'` (or add it to your environment variables in an equivalent fashion). Run the app with `$ node main.js`.
